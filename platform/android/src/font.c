...
#define  IMGDATA_NAME "com/ejoy2dx/" STR_VALUE(PROJECT_NAME) "/ImageData"
#define  IMGDATA_ARG "([BII)Lcom/ejoy2dx/" STR_VALUE(PROJECT_NAME) "/ImageData;"
...
void font_size(const char *str, int unicode, struct font_context * ctx)
{
	...
	if (getStaticMethodInfo(&methodInfo, CLASS_NAME, "getTextImage", IMGDATA_ARG) < 0) {
		return;
	}
	...
	if (jImageData != NULL)
    {
		jclass cls_imgdata = (*methodInfo.env)->FindClass(methodInfo.env,  IMGDATA_NAME);
