...
void
md5_signature_byte(const char * text, size_t sz, size_t sig[4]) {
	MD5_CTX md5;
	...
	MD5Update(&md5, (const unsigned char *)text,sz);
	MD5Final(&md5,decrypt);
    memcpy(&sig[0], &decrypt[0], sizeof(unsigned char) * 16);
}

void 
md5_signature(const char * text, size_t sz, char sig[32]) {
	MD5_CTX md5;
	unsigned char decrypt[16]; 
	int i;
	MD5Init(&md5);
...
void
md5_signature_header(const char * text, size_t sz, const char * header, size_t hsz, char sig[32]) {
	MD5_CTX md5;
	unsigned char decrypt[16]; 
	int i;
	MD5Init(&md5);
	...
	MD5Update(&md5, (const unsigned char *)header,hsz);
	MD5Update(&md5, (const unsigned char *)text,sz);
	MD5Final(&md5,decrypt);
	for (i=0;i<16;i++) {
		sig[i*2] = _hex[decrypt[i] >> 4];
...
//int
//md5_file(const char* filename, char sig[32])
//{
//	int i;
//	FILE* fp;
//	MD5_CTX md5;
...
md5_file(const char* filename, char sig[32])
{
  ...
  MD5Init(&md5);
  while(!pf_feof(fh)) {
    size_t size = pf_fread(buffer, sizeof(unsigned char), 4096, fh);
    MD5Update(&md5, buffer, size);
  }
  MD5Final(&md5, decrypt);
  pf_fileclose(fh);
