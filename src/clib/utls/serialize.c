...
_get_number(lua_State *L, struct read_block *rb, int cookie) {
	...
	case 1: {
		uint8_t n = 0;
		uint8_t * pn = (uint8_t*)rb_read(rb,&n,1);
		if (pn == NULL)
	...
	case 2: {
		uint16_t n = 0;
		uint16_t * pn = (uint16_t*)rb_read(rb,&n,2);
		if (pn == NULL)
			_invalid_stream(L,rb);
		return *pn;
	}
	...
	case 8: {
		double n = 0;
		double * pn = (double*)rb_read(rb,&n,8);
		if (pn == NULL)
			_invalid_stream(L,rb);
		return *pn;
	}
...
static int
_dump(lua_State *L) {
	struct block *b = (struct block*)lua_touserdata(L,1);
	if (b==NULL) {
...
seristring(lua_State *L) {
	...
	lua_pushlstring(L, (char*)buffer, sz);
	free(buffer);
	return 1;
}
