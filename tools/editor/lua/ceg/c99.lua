...
--		other implementation-deÞned characters 
...
argument_expression_list = V"assignment_expression" * (SS * V"," * SS * V"assignment_expression")
...
AND_expression = V"equality_expression" * SS * (V"&" * SS * V"equality_expression")^0
...
--		storage-class-speciÞer declaration-specifiers(opt )
...
--		_Bool 
--		_Complex 
--		struct-or-union-speciÞer
--		enum-speciÞer 
--		typedef-name
type_specifier = V"typedef_name" --(interferes by usurping from declarator), need to track in table
			   + V"struct_or_union_specifier"
			   + V"enum_specifier"
				 + V"void" + V"char" + V"short" + V"int" + V"long" 
...
struct_declaration_list = ((V"anony_struct_or_union" + V"struct_declaration") * SS)^1

-- struct-declaration: 
--		specifier-qualifier-list struct-declarator-list ;
struct_declaration = V"specifier_qualifier_list" * SS * V"struct_declarator_list" * SS * V";"

-- specifier-qualiÞer-list: 
...
--		restrict 
--		volatile
type_qualifier = V"const" + V"restrict" + V"volatile"
...
declarator = (SS * V"pointer")^-1 * SS * V"direct_declarator"
...
-- type-qualiÞer-list: 
--	type-qualifier 
--	type-qualifier-list type-qualiÞer
...
-- identifier-list: 
--		identifier 
--		identifier-list , identiÞer
identifier_list = V"identifier" * (SS * V"," * SS * V"identifier")^0

-- type-name: 
--		specifier-qualiÞer-list abstract-declarator(opt)
...
function test_typedef(match, pos)
	...
	-- print("***************************************")
	-- print(match)
	-- print("test_typedef", str, pos, typename, typedefs[typename])
	-- print("***************************************")
	-- if pos == 60 then
...
--	return nil
	if(typedefs[typename]) then
		...
		if unknown_types then
			table.insert(unknown_types, typename)
		end
		return nil
	end
end
...
typeddef_identitifer = V"identifier"
typedef_type = V"type_specifier" * space *
								(V"typedef_type")^-1
typedef_declarator = V"typedef"*SS*V"typedef_type"*SS*V"typeddef_identitifer"*SS*";"
...
declaration_rules = {
	...
	struct_or_union_definition = struct_or_union_definition,
	start_anony_struct_or_union = start_anony_struct_or_union,
	anony_struct_or_union = anony_struct_or_union,
	...
	typeddef_identitifer = typeddef_identitifer,
	typedef_declarator = typedef_declarator,
